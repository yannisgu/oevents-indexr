...
function indexResults(results, currentIndex, db) {
  ...
  .then((people) => {
    if(people.length > 0) {
      ...
      return db.collection('people').insert({
        name: result.name,
  ...
  .then(person => {
    return db.collection("results").update({"_id": result._id}, {$set:{personId: person._id}}).then(() => { 
      console.log(person)
      if(results.length > (currentIndex + 1)) {
        return indexResults(results, currentIndex + 1, db)
      }
    })
  })
}
